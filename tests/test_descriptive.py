...
def test_confmean():
    ...
    assert_allclose(PyCircStat.confmean(data, xi=0.2, axis=0), out1, rtol=1e-4)
    assert_allclose(PyCircStat.confmean(data, xi=0.2, axis=1), out2, rtol=1e-4)
...
def test_mean_ci():
    ...
    mu_tmp, muplus_tmp, muminus_tmp = PyCircStat.mean_ci(data, xi=0.05, axis=0)
