...
def test_sample_cdf():
    alpha = np.asarray([3.427109860970, 0.649035328217, 0.478105054362, 3.585976113724,
                        2.436564305605, 2.397389764489, 0.223860727758, 3.810441709350,
                        2.194680923763, 5.423136274255])

    pos, cdf = _sample_cdf(alpha, resolution=5)
    pos0 = np.asarray([0.000000000000, 1.256637061436, 2.513274122872, 3.769911184308, 5.026548245744])
    cdf0 = np.asarray([0.300000000000, 0.600000000000, 0.800000000000, 0.900000000000, 1.000000000000])

    assert_allclose(pos, pos0, rtol=1e-4, atol=1e-4, err_msg="Error in evaluated positions.")
    assert_allclose(cdf, cdf0, rtol=1e-4, atol=1e-4, err_msg="Error in evaluated cdfs.")

def test_kuiper_warning():
    alpha1 =np.asarray([0.291662278945, 5.899415544666, 5.402236718096, 3.728212505263, 5.303188109786,
                        3.737946900082, 3.850015526787, 4.902154536516, 3.631621444982, 5.341562525096])
    alpha2 =np.asarray([0.613650458799, 2.109660249330, 3.617555161298, 6.196794760548, 1.856071575830,
                        2.991480015107, 1.789200626487, 4.835921843822, 2.767491245457, 1.744565591973])
    p0 =  0.1
    k0 =  70
...
def test_kuiper():
    ...
    assert_allclose(k, k0, rtol=1e-4, atol=1e-4, err_msg="Error in statistic k.")
    ...
    with warnings.catch_warnings():
        ...
        for i in range(data1.shape[1]):
            for j in range(data1.shape[2]):
                p2, k2 = pycircstat.tests.kuiper(data1[:,i,j], data2[:,i,j])
                assert_equal(p[i,j], p2)
